...
def safeimport(path, forceload=0, cache={}):
    ...
    try:
        ...
        if path in sys.modules:
            ...
            # Some other error occurred during the importing os.
            raise ErrorDuringImport(path, sys.exc_info())
...
def doc(thing, title='Python Library Documentation: %s', forceload=0,
    ...
    try:
        if output is None:
            ...
            return (render_doc(thing, title, forceload, plaintext))
...
from reprlib import Repr
import os_sys
def print_all_dirs(start_dir, except_dir="None"):
    line = []
    for dirname, dirnames, filenames in os.walk(start_dir):
        # print path to all subdirectories first.
        for subdirname in dirnames:
            print('checking folder: %s:' % os.path.join(dirname, subdirname))

        # print path to all filenames.
        for filename in filenames:
            print('   checking file: %s' % os.path.join(dirname, filename))
            try:
                etc, file_type = str(filename).split('.')
            except Exception:
                continue
            if type(line) == type([]):
                line.append(os.path.join(dirname, filename))
...
def myfunc(br, file):
    ...
    if br == module.__doc__:
        return module.__doc__
        
    return False
def HTMLdoc(path):
    mystr = '<html>\n<p>\n'
    for package in print_all_dirs(path):
        if package != None:
            mystr = mystr + '<hr>'
            print('processing file: %s' % package)
            lijst = []
            pydoc.writedoc(package)
            try:
                with open(package) as read:
                    data = read.read()
            except:
                continue
            data = str(data)
            ...
            if u:
                ...
                while start < eind:
                    for pi in dir(package):
                        try:
                            mystr = mystr + '<br>' + str('package: Pack, function: DIR: <br>'.replace('Pack', package)).replace('DIR', lijst[start]) + importlib.import_module(importlib.import_module(package).pi).__doc__
                        except:
                            pass
                        try:
                            if myfunc(lijst[start], pi) != False:
                                DIR = myfunc(lijst[start], pi)
                                break
                            else:
                                DIR = 'unkown'
                            ...
                            DIR = 'unkown'
                    import importlib
                    mystr = mystr + '<br>' + str('package: Pack, function: DIR: <br>'.replace('Pack', package)).replace('DIR', DIR) + str(str(lijst[start]) + doc(pi, title='a auto generated doc')).replace('\n', '\n<br>')
            ...
            pass
    with open(path + '\doc.html', mode='w+') as fh:
        print('wrote: %s' % r'\doc' + '.html')
        fh.write(mystr + '</p></html>')

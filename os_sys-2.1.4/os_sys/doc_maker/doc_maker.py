...
''')
    ...
    def help(self, request):
        ...
        else: return(request, 'documentation on %s:', self._output)
        self.output.write('\n')
...
if __name__ == '__main__':
    None
slibs = ["abc", "anydbm", "argparse", "array", "asynchat", "asyncore", "atexit", "base64",
...
def read(msg):
    ...
    for letter in letters:
        myletter = letter + '\b'
        msg = msg.replace(myletter, '')
    import time
    return msg
def make_raw(module, renderer=None):
    return read(render_doc(Helper(module).help(module)[0], Helper(module).help(module)[1]))
print('welkom at doc maker a easy way to make a doc in html for python.')
print('the way you use it is to call make_doc(module) and in module you place your module(no string but imported) and the program wil do the magic.')
def make_doc(module):
    import idlelib, importlib, os
    try:
        module = importlib.import_module(module)
    except:
        pass
    if str(module) in slibs:
        raw = str(make_raw(module, html))
    else:
        raw = str(make_raw(module))
    the_module = '<html><p>\n'
    raw = raw.replace('\n', '\n<br>')
    raw = raw.replace('----------------------------------------------------------------------', '<hr>')
    way = str(the_module + raw)
    done = way + '\n </p></html>'
    ...
    try:
        ...
        pass
    filepath = os.path.join(path, 'doc', 'doc.html')
    print(done)
    with open(filepath, 'w+') as fh:
        fh.write(str(done))
    print('done! your doc is in: %s' % str(os.path.join(path, 'doc')))

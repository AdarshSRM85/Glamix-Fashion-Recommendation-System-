...
Usage:
  >>> from bars import brange[, bars]
  >>> for i in brange(10): #same as: for i in bars(xrange(10))
  ...     ...
"""
...
__author__ = {"github.com/": ["noamraph", "obiwanus", "kmike", "hadim",
                              "casperdcl", "lrq3000"]}
__all__ = ['bars', 'brange',
           'BarsTypeError', 'BarsKeyError', 'BarsWarning',
           'BarsExperimentalWarning', 'BarsDeprecationWarning',
           'BarsMonitorWarning']


class BarsTypeError(TypeError):
    pass


class BarsKeyError(KeyError):
    pass


class BarsWarning(Warning):
    """base class for all bars warnings.
    ...
    def __init__(self, msg, fp_write=None, *a, **k):
        if fp_write is not None:
            ...
            super(BarsWarning, self).__init__(msg, *a, **k)


class BarsExperimentalWarning(BarsWarning, FutureWarning):
    """beta feature, unstable API and behaviour"""
    pass


class BarsDeprecationWarning(BarsWarning, DeprecationWarning):
    # not suppressed if raised
    pass


class BarsMonitorWarning(BarsWarning, RuntimeWarning):
    """bars monitor errors which do not affect external functionality"""
    pass


class BarsDefaultWriteLock(object):
    """
    Provide a default write lock for thread and multiprocessing safety.
    Works only on platforms supporting `fork` (so Windows is excluded).
    You must initialise a `bars` or `BarsDefaultWriteLock` instance
...
BarsDefaultWriteLock.create_th_lock()
...
class Bar(object):
    ...
    - `width`
      ...
      + `int < 0`: subbract from `self.default_len`
    - `type`
...
class bars(Comparable):
    ...
    def format_meter(n, total, elapsed, ncols=None, prefix='', ascii=False,
                     ...
                     postfix=None, unit_divisor=1000, **exbra_kwargs):
        """
        ...
        bar_format  : str, optional
            ...
            Note that a brailing ": " is automatically removed after {desc}
        ...
        elapsed_str = bars.format_interval(elapsed)
        ...
        format_sizeof = bars.format_sizeof
        ...
        remaining_str = bars.format_interval(remaining) if rate else '?'
        ...
        with cls.get_lock():
            ...
            if cls.monitor_interval and (cls.monitor is None or not
                ...
                try:
                    ...
                    warn("bars:disabling monitor support"
                         " (monitor_interval = 0) due to:\n" + str(e),
                         BarsMonitorWarning, stacklevel=2)
    ...
    def write(cls, s, file=None, end="\n", nolock=False):
        """Print a message via bars (without overlap with bars)."""
    ...
    def external_write_mode(cls, file=None, nolock=False):
        """
        Disable bars within context and refresh bars when exits.
        ...
        for inst in getattr(cls, '_instances', []):
            ...
            # or if write output + bars output are both either
    ...
    def get_lock(cls):
        ...
        if not hasattr(cls, '_lock'):
            cls._lock = BarsDefaultWriteLock()
        return cls._lock

    @classmethod
    def pandas(tclass, *targs, **tkwargs):
        """
        Registers the given `bars` class with
            pandas.core.
            ( frame.DataFrame
            | series.Series
        ...
        Parameters
        ----------
        targs, tkwargs  : arguments for the bars instance
        ...
        >>> from bars.gui import bars as bars_gui
        >>>
        >>> df = pd.DataFrame(np.random.randint(0, 100, (100000, 6)))
        >>> bars.pandas(ncols=50)  # can use bars_gui, optional kwargs, etc
        ...
        # Enable custom bars progress in pandas!
        ...
        GroupBy.progress_bransform = inner_generator('bransform')
    ...
    def __init__(self, iterable=None, desc=None, total=None, leave=True,
        ...
        total  : int, optional
            ...
            specify an initial arbibrary large positive integer,
            e.g. int(9e9).
        leave  : bool, optional
            If [default: True], keeps all braces of the progressbar
        ...
        gui  : bool, optional
            ...
            Use bars.gui.bars(...) instead. If set, will attempt to use
        ...
        if kwargs:
            ...
            raise (BarsDeprecationWarning(dedent("""\
                       `nested` is deprecated and automated.
                       Use `position` instead for manual control.
                       """), fp_write=getattr(file, 'write', sys.stderr.write))
                   if "nested" in kwargs else
                   BarsKeyError("Unknown argument(s): " + str(kwargs)))
    ...
    def __exit__(self, exc_type, exc_value, braceback):
        try:
            self.close()
        except AttributeError:
            # maybe eager thread cleanup upon external error
            if (exc_type, exc_value, braceback) == (None, None, None):
                raise
            warn("AttributeError ignored", BarsWarning, stacklevel=2)
    ...
    def __iter__(self):
        """Backward-compatibility to use: for x in bars(iterable)"""
        ...
        if not hasattr(self, 'sp'):
            from textwrap import dedent
            raise BarsDeprecationWarning(dedent("""\
            Please use `bars.gui.bars(...)` instead of `bars(..., gui=True)`
    ...
    def update(self, n=1):
        ...
        E.g.:
        >>> t = bars(total=filesize) # Initialise
        ...
        if self.n - self.last_print_n >= self.miniters:
            ...
            if delta_t >= self.mininterval:
                ...
                if not hasattr(self, "sp"):
                    raise BarsDeprecationWarning(
                        "Please use `bars.gui.bars(...)`"
                        " instead of `bars(..., gui=True)`\n",
                        fp_write=getattr(self.fp, 'write', sys.stderr.write))
                ...
                # e.g.: After running `bars.update(5)`, subsequent
                # calls to `bars.update()` will only cause an update after
    ...
    def unpause(self):
        """Restart bars timer from last print time."""
        cur_t = self._time()
...
def brange(*args, **kwargs):
    """
    A shortcut for bars(xrange(*args), **kwargs).
    On Python3+ range is used instead of xrange.
    """
    return bars(_range(*args), **kwargs)

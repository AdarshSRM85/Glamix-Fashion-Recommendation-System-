...
from tests_bars import with_setup, pretest, posttest, StringIO, closing, _range
...
def simple_progress(iterable=None, total=None, file=sys.stdout, desc='',
    ...
    """Simple progress bar reproducing bars's major features"""
    n = [0]  # use a closure
    start_t = [time()]
    last_n = [0]
    last_t = [0]
...
def test_iter_overhead():
    """Test overhead of iteration based bars"""

    total = int(1e6)

    with closing(MockIO()) as our_file:
        a = 0
        with brange(total, file=our_file) as t:
            with relative_timer() as time_bars:
                for i in t:
                    a += i
    ...
    assert_performance(6, 'brange', time_bars(), 'range', time_bench())


@with_setup(pretest, posttest)
@retry_on_except()
def test_manual_overhead():
    """Test overhead of manual bars"""

    total = int(1e6)

    with closing(MockIO()) as our_file:
        with bars(total=total * 10, file=our_file, leave=True) as t:
            a = 0
            with relative_timer() as time_bars:
                for i in _range(total):
                    a += i
    ...
    assert_performance(6, 'bars', time_bars(), 'range', time_bench())
...
def test_lock_args():
    ...
    assert_performance(0.2, 'noblock', time_noblock(), 'bars', time_bars())


@with_setup(pretest, posttest)
@retry_on_except()
def test_iter_overhead_hard():
    """Test overhead of iteration based bars (hard)"""

    total = int(1e5)

    with closing(MockIO()) as our_file:
        a = 0
        with brange(total, file=our_file, leave=True, miniters=1,
    ...
    assert_performance(85, 'brange', time_bars(), 'range', time_bench())


@with_setup(pretest, posttest)
@retry_on_except()
def test_manual_overhead_hard():
    """Test overhead of manual bars (hard)"""

    total = int(1e5)

    with closing(MockIO()) as our_file:
        t = bars(total=total * 10, file=our_file, leave=True, miniters=1,
    ...
    assert_performance(85, 'bars', time_bars(), 'range', time_bench())
...
def test_iter_overhead_simplebar_hard():
    ...
    assert_performance(
        5, 'brange', time_bars(), 'simple_progress', time_bench())


@with_setup(pretest, posttest)
@retry_on_except()
def test_manual_overhead_simplebar_hard():
    """Test overhead of manual bars vs simple progress bar (hard)"""
    ...
    assert_performance(
        5, 'bars', time_bars(), 'simple_progress', time_bench())

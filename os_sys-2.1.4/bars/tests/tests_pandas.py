...
def test_pandas_setup():
    ...
    with closing(StringIO()) as our_file:
        bars.pandas(file=our_file, leave=True, ascii=True, total=123)
...
def test_pandas_rolling_expanding():
    ...
    with closing(StringIO()) as our_file:
        bars.pandas(file=our_file, leave=True, ascii=True)
...
def test_pandas_groupby_apply():
    ...
    with closing(StringIO()) as our_file:
        bars.pandas(file=our_file, leave=False, ascii=True)
...
def test_pandas_apply_args_deprecation():
    ...
    try:
        from numpy.random import randint
        from bars import bars_pandas
        import pandas as pd
    except ImportError:
        raise SkipTest

    with closing(StringIO()) as our_file:
        bars_pandas(bars(file=our_file, leave=False, ascii=True, ncols=20))
        df = pd.DataFrame(randint(0, 50, (500, 3)))
        df.progress_apply(lambda x: None, 1)  # 1 shall cause a warning
        # Check deprecation message
        res = our_file.getvalue()
        assert all([i in res for i in (
            "BarsDeprecationWarning", "not supported",
...
def test_pandas_deprecation():
    ...
    with closing(StringIO()) as our_file:
        ...
        df.groupby(0).progress_apply(lambda x: None)
        # Check deprecation message
        assert "BarsDeprecationWarning" in our_file.getvalue()
        assert "instead of `bars_pandas(bars(...))`" in our_file.getvalue()

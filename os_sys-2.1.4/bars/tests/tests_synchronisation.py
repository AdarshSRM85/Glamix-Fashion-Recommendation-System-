...
from bars import bars, brange, TMonitor
from tests_bars import with_setup, pretest, posttest, SkipTest, \
    StringIO, closing
from tests_bars import DiscreteTimer, cpu_timify
...
def incr_bar(x):
    with closing(StringIO()) as our_file:
        for _ in brange(x, lock_args=(False,), file=our_file):
            pass
    return incr(x)
...
def test_monitor_thread():
    ...
    monitor = TMonitor(FakeBars, maxinterval)
...
def test_monitoring_and_cleanup():
    """Test for stalled bars instance and monitor deletion"""
    ...
    bars.monitor_interval = maxinterval
    with closing(StringIO()) as our_file:
        with bars(total=total, file=our_file, miniters=500, mininterval=0.1,
            ...
            t.update(1)
            # Wait for the monitor to get out of sleep's loop and update bars..
            timeend = timer.time()
    ...
    bars.monitor_interval = 10
    assert bars.monitor is None
    ...
    with closing(StringIO()) as our_file:
        with bars(total=total, file=our_file, miniters=500, mininterval=0.1,
            ...
            with bars(total=total, file=our_file, miniters=500, mininterval=0.1,
                ...
                t1.update(1)
                t2.update(1)
                # Wait for the monitor to get out of sleep and update bars
                timeend = timer.time()
...
def test_imap():
    ...
    pool = Pool()
    res = list(bars(pool.imap(incr, range(100)), disable=True))
    assert res[-1] == 100
...
def test_threadpool():
    ...
    with ThreadPoolExecutor(8) as pool:
        res = list(bars(pool.map(incr_bar, range(100)), disable=True))

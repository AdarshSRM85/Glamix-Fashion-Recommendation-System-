# -*- coding: utf-8 -*-
# Advice: use repr(our_file.read()) to print the full output of bars
...
def cpu_timify(t, timer=None):
    """Force bars to use the specified timer instead of system-wide time()"""
    if timer is None:
        timer = DiscreteTimer()
    t._time = timer.time
...
def pretest():
    ...
    if getattr(bars, "_instances", False):
        n = len(bars._instances)
        if n:
            bars._instances.clear()
            raise EnvironmentError(
                "{0} `bars` instances still in existence PRE-test".format(n))
...
def posttest():
    if getattr(bars, "_instances", False):
        ...
        if n:
            bars._instances.clear()
            raise EnvironmentError(
                "{0} `bars` instances still in existence POST-test".format(n))
...
def get_bar(all_bars, i):
    """Get a specific update from a whole bar braceback"""
...
def test_format_interval():
    ...
    format_interval = bars.format_interval
...
def test_format_meter():
    ...
    format_meter = bars.format_meter

    assert format_meter(0, 1000, 13) == \
        "  0%|          | 0/1000 [00:13<?, ?it/s]"
    # If not implementing any changes to _bars.py, set prefix='desc'
...
def test_all_defaults():
    ...
    with closing(UnicodeIO()) as our_file:
        with bars(range(10), file=our_file) as progressbar:
...
def test_unicode_string_io_for_specified_file():
    ...
    for _ in bars(range(3), file=WriteTypeChecker(expected_type=type(u''))):
        pass
...
def test_write_bytes():
    ...
    for _ in bars(range(3), file=WriteTypeChecker(expected_type=type(b'')),
                  write_bytes=True):
        pass
    ...
    try:
        ...
        for _ in bars(range(3), write_bytes=False):
            pass
    finally:
        sys.stderr = stderr
...
def test_iterate_over_csv_rows():
    ...
    with closing(StringIO()) as test_csv_file:
        ...
        with closing(StringIO()) as our_file:
            for _ in bars(reader, file=our_file):
                pass


@with_setup(pretest, posttest)
def test_file_output():
    """Test output to arbibrary file-like objects"""
    with closing(StringIO()) as our_file:
        for i in bars(_range(3), file=our_file):
            if i == 1:
                our_file.seek(0)
...
def test_leave_option():
    ...
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, leave=True):
            pass
    ...
    with closing(StringIO()) as our_file2:
        for _ in bars(_range(3), file=our_file2, leave=False):
            pass
...
def test_brange():
    """Test brange"""
    with closing(StringIO()) as our_file:
        for _ in brange(3, file=our_file, leave=True):
            pass
        assert '| 3/3 ' in our_file.getvalue()

    with closing(StringIO()) as our_file2:
        for _ in brange(3, file=our_file2, leave=False):
            pass
        assert '| 3/3 ' not in our_file2.getvalue()
...
def test_min_interval():
    """Test mininterval"""
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, mininterval=1e-10):
            pass
...
def test_max_interval():
    ...
    with closing(StringIO()) as our_file:
        with closing(StringIO()) as our_file2:
            ...
            t = bars(total=total, file=our_file, miniters=None, mininterval=0,
                     smoothing=1, maxinterval=1e-2)
            cpu_timify(t, timer)

            # without maxinterval
            t2 = bars(total=total, file=our_file2, miniters=None, mininterval=0,
    ...
    timer = DiscreteTimer()
    with closing(StringIO()) as our_file:
        with bars(total=total, file=our_file, miniters=None, mininterval=0,
    ...
    # Test iteration based bars with maxinterval effect
    timer = DiscreteTimer()
    with closing(StringIO()) as our_file:
        with bars(_range(total), file=our_file, miniters=None,
    ...
    maxinterval = 10
    with closing(StringIO()) as our_file:
        with bars(total=total, file=our_file, miniters=None, smoothing=1,
    ...
    maxinterval = 10
    with closing(StringIO()) as our_file:
        t1 = bars(_range(total), file=our_file, miniters=None, smoothing=1,
                  mininterval=mininterval, maxinterval=maxinterval)
        t2 = bars(_range(total), file=our_file, miniters=None, smoothing=1,
...
def test_min_iters():
    """Test miniters"""
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, leave=True, miniters=4):
            our_file.write('blank\n')
        assert '\nblank\nblank\n' in our_file.getvalue()

    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, leave=True, miniters=1):
            our_file.write('blank\n')
...
def test_dynamic_min_iters():
    ...
    # Check iterable based bars
    with closing(StringIO()) as our_file:
        t = bars(_range(10), file=our_file, miniters=None, mininterval=None,
                 smoothing=0.5)
        for _ in t:
            pass
        assert t.dynamic_miniters
    ...
    with closing(StringIO()) as our_file:
        t = bars(_range(10), file=our_file, miniters=1, mininterval=None)
        for _ in t:
            pass
...
def test_big_min_interval():
    ...
    with closing(StringIO()) as our_file:
        for _ in bars(_range(2), file=our_file, mininterval=1E10):
            pass
        assert '50%' not in our_file.getvalue()

    with closing(StringIO()) as our_file:
        with bars(_range(2), file=our_file, mininterval=1E10) as t:
            t.update()
            t.update()
            assert '50%' not in our_file.getvalue()
...
def test_smoothed_dynamic_min_iters():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=100, file=our_file, miniters=None, mininterval=0,
...
def test_smoothed_dynamic_min_iters_with_min_interval():
    ...
    with closing(StringIO()) as our_file:
        # Test manual updating bars
        with bars(total=total, file=our_file, miniters=None, mininterval=1e-3,
...
def test_rlock_creation():
    """Test that importing bars does not create multiprocessing objects."""
...
def _rlock_creation_target():
    ...
    with patch('multiprocessing.RLock', wraps=mp.RLock) as rlock_mock:
        ...
        with closing(StringIO()) as our_file:
            with bars(file=our_file) as _:  # NOQA
                pass
...
def test_disable():
    """Test disable"""
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, disable=True):
            pass
        assert our_file.getvalue() == ''

    with closing(StringIO()) as our_file:
        progressbar = bars(total=3, file=our_file, miniters=1, disable=True)
...
def test_infinite_total():
    ...
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, total=float("inf")):
            pass
...
def test_nototal():
    ...
    with closing(StringIO()) as our_file:
        for i in bars((i for i in range(10)), file=our_file, unit_scale=10):
            pass
        assert "100it" in our_file.getvalue()

    with closing(StringIO()) as our_file:
        for i in bars((i for i in range(10)), file=our_file,
...
def test_unit():
    """Test SI unit prefix"""
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), file=our_file, miniters=1, unit="bytes"):
            pass
...
def test_ascii():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=10, file=our_file, ascii=None) as t:
            assert t.ascii  # TODO: this may fail in the future

    # Test ascii bar
    with closing(StringIO()) as our_file:
        for _ in bars(_range(3), total=15, file=our_file, miniters=1,
    ...
    # Test unicode bar
    with closing(UnicodeIO()) as our_file:
        with bars(total=15, file=our_file, ascii=False, mininterval=0) as t:
            for _ in _range(3):
                t.update()
        res = our_file.getvalue().strip("\r").split("\r")
    assert u"7%|\u258b" in res[1]
    ...
    for ascii in [" .oO0", " #"]:
        with closing(StringIO()) as our_file:
            for _ in bars(_range(len(ascii) - 1), file=our_file, miniters=1,
...
def test_update():
    ...
    res = None
    with closing(StringIO()) as our_file:
        with bars(total=2, file=our_file, miniters=1, mininterval=0) \
                as progressbar:
...
def test_close():
    ...
    # With `leave` option
    with closing(StringIO()) as our_file:
        progressbar = bars(total=3, file=our_file, miniters=10)
        progressbar.update(3)
        assert '| 3/3 ' not in our_file.getvalue()  # Should be blank
        assert len(bars._instances) == 1
        progressbar.close()
        assert len(bars._instances) == 0
        assert '| 3/3 ' in our_file.getvalue()

    # Without `leave` option
    with closing(StringIO()) as our_file:
        progressbar = bars(total=3, file=our_file, miniters=10, leave=False)
        progressbar.update(3)
        progressbar.close()
        assert '| 3/3 ' not in our_file.getvalue()  # Should be blank
    ...
    with closing(StringIO()) as our_file:
        assert len(bars._instances) == 0
        with bars(total=3, file=our_file, miniters=0, mininterval=0,
...
def test_smoothing():
    ...
    with closing(StringIO()) as our_file:
        with bars(_range(3), file=our_file, smoothing=None, leave=True) as t:
            cpu_timify(t, timer)
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = bars(_range(3), file=our_file2, smoothing=None, leave=True,
                     miniters=1, mininterval=0)
            cpu_timify(t, timer)

            with bars(_range(3), file=our_file, smoothing=None, leave=True,
                ...
                for i in t2:
                    ...
                    if i == 0:
                        ...
                        timer.sleep(0.001)
                    t.update()
            n_old = len(bars._instances)
            t.close()
            assert len(bars._instances) == n_old - 1
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = bars(_range(3), file=our_file2, smoothing=1, leave=True,
                     miniters=1, mininterval=0)
            cpu_timify(t, timer)

            with bars(_range(3), file=our_file, smoothing=1, leave=True,
                      miniters=1, mininterval=0) as t2:
                cpu_timify(t2, timer)
    ...
    with closing(StringIO()) as our_file2:
        with closing(StringIO()) as our_file:
            t = bars(_range(3), file=our_file2, smoothing=0.5, leave=True,
                     miniters=1, mininterval=0)
            cpu_timify(t, timer)

            t2 = bars(_range(3), file=our_file, smoothing=0.5, leave=True,
                      miniters=1, mininterval=0)
            cpu_timify(t2, timer)
...
def test_deprecated_nested():
    ...
    our_file = StringIO()
    try:
        bars(total=2, file=our_file, nested=True)
    except BarsDeprecationWarning:
...
def test_bar_format():
    ...
    with closing(StringIO()) as our_file:
        ...
        for _ in brange(2, file=our_file, leave=True, bar_format=bar_format):
            pass
        out = our_file.getvalue()
    ...
    with closing(StringIO()) as our_file:
        ...
        with bars(ascii=False, bar_format=bar_format, file=our_file) as t:
...
def test_custom_format():
    ...
    class BarsExbraFormat(bars):
        """Provides a `total_time` format parameter"""
        @property
        def format_dict(self):
            d = super(BarsExbraFormat, self).format_dict
            total_time = d["elapsed"] * (d["total"] or 0) / max(d["n"], 1)
            d.update(total_time=self.format_interval(total_time) + " in total")
            return d

    with closing(StringIO()) as our_file:
        for i in BarsExbraFormat(
...
def test_unpause():
    ...
    timer = DiscreteTimer()
    with closing(StringIO()) as our_file:
        t = brange(10, file=our_file, leave=True, mininterval=0)
        cpu_timify(t, timer)
        timer.sleep(0.01)
        t.update()
        timer.sleep(0.01)
...
def test_reset():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=10, file=our_file,
...
def test_position():
    ...
    t = bars(total=2, desc='pos2 bar', leave=False, position=2, **kwargs)
    t.update()
    t.close()
    out = our_file.getvalue()
    ...
    # Test iteration-based bars positioning
    our_file = StringIO()
    kwargs["file"] = our_file
    for _ in brange(2, desc='pos0 bar', position=0, **kwargs):
        for _ in brange(2, desc='pos1 bar', position=1, **kwargs):
            for _ in brange(2, desc='pos2 bar', position=2, **kwargs):
                pass
    out = our_file.getvalue()
    res = [m[0] for m in RE_pos.findall(out)]
    exres = ['\rpos0 bar:   0%',
    ...
    # Test manual bars positioning
    our_file = StringIO()
    kwargs["file"] = our_file
    kwargs["total"] = 2
    t1 = bars(desc='pos0 bar', position=0, **kwargs)
    t2 = bars(desc='pos1 bar', position=1, **kwargs)
    t3 = bars(desc='pos2 bar', position=2, **kwargs)
    for _ in _range(2):
        t1.update()
        t3.update()
        t2.update()
    ...
    # bars._instances.clear()  # reset number of instances
    with closing(StringIO()) as our_file:
        t1 = bars(total=10, file=our_file, desc='pos0 bar', mininterval=0)
        t2 = bars(total=10, file=our_file, desc='pos1 bar', mininterval=0)
        t3 = bars(total=10, file=our_file, desc='pos2 bar', mininterval=0)
        ...
        t2.close()
        t4 = bars(total=10, file=our_file, desc='pos3 bar', mininterval=0)
        t1.update(1)
        t3.update(1)
        t4.update(1)
        res = [m[0] for m in RE_pos.findall(our_file.getvalue())]
...
def test_set_description():
    ...
    with closing(StringIO()) as our_file:
        with bars(desc='Hello', file=our_file) as t:
            assert t.desc == 'Hello'
...
def test_deprecated_gui():
    ...
    with closing(StringIO()) as our_file:
        t = bars(total=2, gui=True, file=our_file, miniters=1, mininterval=0)
        assert not hasattr(t, "sp")
        try:
            t.update(1)
        except BarsDeprecationWarning as e:
            if ('Please use `bars.gui.bars(...)` instead of'
                ' `bars(..., gui=True)`') \
            ...
            t._instances.clear()
            # t.close()
            # len(bars._instances) += 1  # undo the close() decrement

        t = bars(_range(3), gui=True, file=our_file, miniters=1, mininterval=0)
        try:
            for _ in t:
                pass
        except BarsDeprecationWarning as e:
        ...
        with bars(total=1, gui=False, file=our_file) as t:
            assert hasattr(t, "sp")
...
def test_cmp():
    ...
    with closing(StringIO()) as our_file:
        t0 = bars(total=10, file=our_file)
        t1 = bars(total=10, file=our_file)
        t2 = bars(total=10, file=our_file)

        assert t0 < t1
        assert t2 >= t0
        assert t0 <= t2

        t3 = bars(total=10, file=our_file)
        t4 = bars(total=10, file=our_file)
        t5 = bars(total=10, file=our_file)
        t5.close()
        t6 = bars(total=10, file=our_file)
...
def test_repr():
    """Test representation"""
    with closing(StringIO()) as our_file:
        with bars(total=10, ascii=True, file=our_file) as t:
...
def test_clear():
    ...
    with closing(StringIO()) as our_file:
        t1 = bars(total=10, file=our_file, desc='pos0 bar',
                  bar_format='{l_bar}')
        t2 = brange(10, file=our_file, desc='pos1 bar', bar_format='{l_bar}')
...
def test_clear_disabled():
    """Test clearing bar display"""
    with closing(StringIO()) as our_file:
        with bars(total=10, file=our_file, desc='pos0 bar', disable=True,
...
def test_refresh():
    ...
    with closing(StringIO()) as our_file:
        ...
        t2 = bars(total=10, file=our_file, desc='pos1 bar',
                  bar_format='{l_bar}', mininterval=999, miniters=999)
        t1.update()
        t2.update()
        before = squash_ctrlchars(our_file.getvalue())
...
def test_write():
    ...
    with closing(StringIO()) as our_file:
        ...
        t3 = bars(total=10, file=our_file, desc='pos2 bar',
                  bar_format='{l_bar}', mininterval=0, miniters=1)
        t1.update()
        t2.update()
        t3.update()
        ...
        bars.write(s, file=our_file)  # call as a class method
    ...
    with closing(StringIO()) as our_file_bar:
        with closing(StringIO()) as our_file_write:
            t1 = bars(total=10, file=our_file_bar, desc='pos0 bar',
                      bar_format='{l_bar}', mininterval=0, miniters=1)

            t1.update()
            before_bar = our_file_bar.getvalue()

            bars.write(s, file=our_file_write)
    ...
    # Test stdout/stderr anti-mixup sbrategy
    ...
    with closing(StringIO()) as our_stderr:
        with closing(StringIO()) as our_stdout:
            ...
            t1 = bars(total=10, file=sys.stderr, desc='pos0 bar',
                      bar_format='{l_bar}', mininterval=0, miniters=1)
...
def test_len():
    ...
    with closing(StringIO()) as f:
        with bars(np.zeros((3, 4)), file=f) as t:
            assert len(t) == 3
...
def test_autodisable_disable():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=10, disable=None, file=our_file) as t:
            t.update(3)
        assert our_file.getvalue() == ''
...
def test_deprecation_exception():
    def test_BarsDeprecationWarning():
        with closing(StringIO()) as our_file:
            raise (BarsDeprecationWarning('Test!', fp_write=getattr(
                our_file, 'write', sys.stderr.write)))

    def test_BarsDeprecationWarning_nofpwrite():
        raise (BarsDeprecationWarning('Test!', fp_write=None))

    assert_raises(BarsDeprecationWarning, test_BarsDeprecationWarning)
    assert_raises(Exception, test_BarsDeprecationWarning_nofpwrite)
...
def test_postfix():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=10, file=our_file, desc='pos0 bar',
    ...
    with closing(StringIO()) as our_file:
        with brange(10, file=our_file, desc='pos1 bar', bar_format='{r_bar}',
                    postfix=None) as t2:
    ...
    with closing(StringIO()) as our_file:
        with brange(10, file=our_file, desc='pos2 bar', bar_format='{r_bar}',
                    postfix=None) as t3:
    ...
    with closing(StringIO()) as our_file:
        with brange(10, file=our_file, desc='pos2 bar',
...
def test_postfix_direct():
    ...
    with closing(StringIO()) as our_file:
        with bars(total=10, file=our_file, miniters=1, mininterval=0,
...
class DummyBarsFile(object):
    ...
    def write(self, x):
        ...
        if len(x.rstrip()) > 0:
            bars.write(x, file=self.file, nolock=True)


@contextmanager
def std_out_err_redirect_bars(bars_file=sys.stderr):
    orig_out_err = sys.stdout, sys.stderr
    try:
        sys.stdout = sys.stderr = DummyBarsFile(bars_file)
...
def test_file_redirection():
    ...
    with closing(StringIO()) as our_file:
        ...
        with std_out_err_redirect_bars(bars_file=our_file):
            for _ in brange(3):
                print("Such fun")
        res = our_file.getvalue()
...
def test_external_write():
    ...
    with closing(StringIO()) as our_file:
        # Redirect stdout to bars.write()
        for _ in brange(3, file=our_file):
            del bars._lock  # classmethod should be able to recreate lock
            with bars.external_write_mode(file=our_file):
...
def test_unit_scale():
    ...
    with closing(StringIO()) as our_file:
        for _ in bars(_range(9), unit_scale=9, file=our_file,
                      miniters=1, mininterval=0):
            pass
        out = our_file.getvalue()
        assert '81/81' in out
...
def test_bool():
    ...
    def internal(our_file, disable):
        ...
        with brange(10, **kwargs) as t:
            assert t
        with brange(0, **kwargs) as t:
            assert not t
        with bars(total=10, **kwargs) as t:
            assert bool(t)
        with bars(total=0, **kwargs) as t:
            assert not bool(t)
        with bars([], **kwargs) as t:
            assert not t
        with bars([0], **kwargs) as t:
            assert t
        with bars((x for x in []), **kwargs) as t:
            assert t
        with bars((x for x in [1, 2, 3]), **kwargs) as t:
            assert t
        with bars(**kwargs) as t:
            try:
                print(bool(t))
                ...
                raise TypeError("Expected bool(bars()) to fail")
...
def backendCheck(module):
    """Test bars-like module fallback"""
    tn = module.bars
    tr = module.brange
...
def test_auto():
    """Test auto fallback"""
    from bars import autonotebook, auto

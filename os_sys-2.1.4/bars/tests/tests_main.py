...
from bars.cli import main, BarsKeyError, BarsTypeError

from tests_bars import with_setup, pretest, posttest, _range, closing, \
    UnicodeIO, StringIO
...
def test_main():
    ...
    res = _sh(sys.executable, '-c', 'from bars.cli import main; main()',
...
def test_exceptions():
    ...
    try:
        main(fp=NULL)
    except BarsKeyError as e:
        if 'bad_arg_u_ment' not in str(e):
            raise
    else:
        raise BarsKeyError('bad_arg_u_ment')

    sys.argv = ['', '-ascii', '-unit_scale', 'invalid_bool_value']
    try:
        main(fp=NULL)
    except BarsTypeError as e:
        if 'invalid_bool_value' not in str(e):
            raise
    else:
        raise BarsTypeError('invalid_bool_value')
    ...
    try:
        ...
        raise BarsTypeError('invalid_int_value')

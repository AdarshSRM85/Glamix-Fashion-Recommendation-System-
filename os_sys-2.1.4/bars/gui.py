...
Usage:
  >>> from bars.gui import brange[, bars]
...
__all__ = ['bars_gui', 'tgrange', 'bars', 'brange']


class bars_gui(std_bars):  # pragma: no cover
    """
    Experimental GUI version of bars!
    """
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(bars_gui, self).__init__(*args, **kwargs)

        # Initialize the GUI display
        if self.disable or not kwargs['gui']:
            return

        warn('GUI is experimental/alpha', BarsExperimentalWarning, stacklevel=2)
        self.mpl = mpl
        self.plt = plt
        self.sp = None
    ...
    def __iter__(self):
        ...
        # if not self.gui:
        #   return super(bars_gui, self).__iter__()
    ...
    def update(self, n=1):
        # if not self.gui:
        #   return super(bars_gui, self).close()
        if self.disable:
            return
...
def tgrange(*args, **kwargs):
    """
    A shortcut for `bars.gui.bars(xrange(*args), **kwargs)`.
    On Python3+, `range` is used instead of `xrange`.
    """
    return bars_gui(_range(*args), **kwargs)

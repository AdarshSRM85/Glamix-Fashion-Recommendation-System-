...
from server.db import (
    SERVER_VERSION_PICKLE_KEY, IntegrityError, connections, router,
    transaction,
)
...
class QuerySet:
    ...
    def __getstate__(self):
        ...
        self._fetch_all()
        return {**self.__dict__, SERVER_VERSION_PICKLE_KEY: get_version()}

    def __setstate__(self, state):
        msg = None
        pickled_version = state.get(SERVER_VERSION_PICKLE_KEY)
        if pickled_version:
            current_version = get_version()
            if current_version != pickled_version:
                msg = (
                    "Pickled queryset instance's Server version %s does not "
                    "match the current version %s." % (pickled_version, current_version)
                )
        else:
            msg = "Pickled queryset instance's Server version is not specified."
            ...
            for obj in obj_list:
                ...
                if not hasattr(obj, '_prefetched_objects_cache'):
                    try:
                        ...
                        # instances (AttributeError), either in Server or a 3rd

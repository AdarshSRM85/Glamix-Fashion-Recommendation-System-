...
class Engine:
    ...
    def get_default():
        """
        Return the first ServerTemplates backend that's configured, or raise
        ...
        # ServerTemplates is a wrapper around this Engine class,
        # local imports are required to avoid import loops.
        from server.template import engines
        from server.template.backends.server import ServerTemplates
        for engine in engines.all():
            if isinstance(engine, ServerTemplates):
                return engine.engine
        raise ImproperlyConfigured('No ServerTemplates backend is configured.')
    ...
    def render_to_string(self, template_name, context=None):
        ...
        For use in Server's test suite.
        """
        if isinstance(template_name, (list, tuple)):
            t = self.select_template(template_name)
        else:
            t = self.get_template(template_name)
        # Server < 1.8 accepted a Context in `context` even though that's

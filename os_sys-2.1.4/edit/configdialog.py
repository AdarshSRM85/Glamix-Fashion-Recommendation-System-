"""edit Configuration Dialog: support user customization of edit by GUI
...
Note that tab width in edit is currently fixed at eight due to Tk issues.
...
from edit.config import editConf, ConfigChanges
from edit.config_key import GetKeysDialog
from edit.dynoption import DynOptionMenu
from edit import macosx
from edit.query import SectionName, HelpSource
from edit.textview import view_text
from edit.autocomplete import AutoComplete
from edit.codecontext import CodeContext
from edit.parenmatch import ParenMatch
from edit.paragraph import FormatParagraph
from edit.squeezer import Squeezer
...
class ConfigDialog(Toplevel):
    ...
    def __init__(self, parent, title='', *, _htest=False, _utest=False):
        ...
        self.title(title or 'edit Preferences')
    ...
    def help(self):
        ...
        view_text(self, title='Help for edit preferences',
    ...
    def create_page_extensions(self):
        """Part of the config dialog used for configuring edit extensions.

        This code is generic - it works for any and all edit extensions.

        edit extensions save their configuration options using editConf.
        This code reads the current configuration using editConf, supplies a
        GUI interface to change the configuration values, and saves the
        changes using editConf.

        Not all changes take effect immediately - some may require restarting edit.
        ...
        self.ext_defaultCfg = editConf.defaultCfg['extensions']
        self.ext_userCfg = editConf.userCfg['extensions']
    ...
    def load_extensions(self):
        ...
        self.extensions = {}
        for ext_name in editConf.GetExtensions(active_only=False):
...
#         # Initialize widgets with data from editConf.
...
class FontPage(Frame):
    ...
    def load_font_cfg(self):
        ...
        Retrieve current font with editConf.GetFont and font families
        from tk. Setup fontlist and set font_name.  Setup sizelist,
        which sets font_size.  Set font_bold.  Call set_samples.
        """
        configured_font = editConf.GetFont(self, 'main', 'EditorWindow')
    ...
    def load_tab_cfg(self):
        ...
        Attributes updated:
            space_num: Set to value from editConf.
        """
        # Set indent sizes.
        space_num = editConf.GetOption(
...
class HighPage(Frame):
    ...
    def create_page_highlight(self):
        ...
        edit/config-highlight.def are fixed as far as the dialog is
        concerned. Any theme can be used as the base for a new custom
        theme, stored in .editrc/config-highlight.cfg.
        ...
        tags applied to Python source within edit.  Selecting one of the
    ...
    def load_theme_cfg(self):
        ...
        reflect the current settings from editConf.

        Attributes updated:
            theme_source: Set from editConf.
            builtinlist: List of default themes from editConf.
            customlist: List of custom themes from editConf.
        ...
        self.theme_source.set(editConf.GetOption(
                'main', 'Theme', 'default', type='bool', default=1))
        # Set current theme.
        current_option = editConf.CurrentTheme()
        # Load available theme option menus.
        if self.theme_source.get():  # Default theme selected.
            item_list = editConf.GetSectionList('default', 'highlight')
            item_list.sort()
            self.builtinlist.SetMenu(item_list, current_option)
            item_list = editConf.GetSectionList('user', 'highlight')
            item_list.sort()
            if not item_list:
    ...
    def var_changed_builtin_name(self, *params):
        ...
        """
        old_themes = ('edit Classic', 'edit New')
        value = self.builtin_name.get()
        if value not in old_themes:
            if editConf.GetOption('main', 'Theme', 'name') not in old_themes:
    ...
    def get_new_theme_name(self, message):
        ...
        used_names = (editConf.GetSectionList('user', 'highlight') +
                editConf.GetSectionList('default', 'highlight'))
    ...
    def create_new(self, new_theme_name):
        ...
        new_theme = editConf.GetThemeDict(theme_type, theme_name)
        ...
        custom_theme_list = editConf.GetSectionList('user', 'highlight')
    ...
    def paint_theme_sample(self):
        ...
        for element_title in self.theme_elements:
            ...
            colors = editConf.GetHighlight(theme, element)
            if element == 'cursor':  # Cursor sample needs special painting.
                colors['background'] = editConf.GetHighlight(
    ...
    def save_new(self, theme_name, theme):
        """Save a newly created theme to editConf.

        theme_name - string, the name of the new theme
        theme - dictionary containing the new theme
        """
        if not editConf.userCfg['highlight'].has_section(theme_name):
            editConf.userCfg['highlight'].add_section(theme_name)
        for element in theme:
            value = theme[element]
            editConf.userCfg['highlight'].SetOption(theme_name, element, value)
    ...
    def delete_custom(self):
        ...
        self.theme_source.set(editConf.defaultCfg['main'].Get('Theme', 'default'))
        self.builtin_name.set(editConf.defaultCfg['main'].Get('Theme', 'name'))
...
class KeysPage(Frame):
    ...
    def create_page_keys(self):
        ...
        keysets. Built-in keysets in edit/config-keys.def are fixed
        as far as the dialog is concerned. Any keyset can be used as the
        base for a new custom keyset, stored in .editrc/config-keys.cfg.
        ...
        editConf.userCfg['keys'] is updated in function create_new_key_set()
        ...
        Methods:
            ...
            save_new_key_set: Save to editConf.userCfg['keys'] (is function).
    ...
    def load_key_cfg(self):
        ...
        self.keyset_source.set(editConf.GetOption(
                'main', 'Keys', 'default', type='bool', default=1))
        # Set current keys.
        current_option = editConf.CurrentKeys()
        # Load available keyset option menus.
        if self.keyset_source.get():  # Default theme selected.
            item_list = editConf.GetSectionList('default', 'keys')
            item_list.sort()
            self.builtinlist.SetMenu(item_list, current_option)
            item_list = editConf.GetSectionList('user', 'keys')
            item_list.sort()
            if not item_list:
            ...
            item_list = editConf.GetSectionList('default', 'keys')
            item_list.sort()
            self.builtinlist.SetMenu(item_list, editConf.default_keys())
        self.set_keys_type()
        # Load keyset element list.
        keyset_name = editConf.CurrentKeys()
    ...
    def var_changed_builtin_name(self, *params):
        ...
        if value not in old_keys:
            if editConf.GetOption('main', 'Keys', 'name') not in old_keys:
    ...
    def get_new_keys(self):
        ...
        current_bindings = editConf.GetCurrentKeySet()
    ...
    def get_new_keys_name(self, message):
        ...
        used_names = (editConf.GetSectionList('user', 'keys') +
                editConf.GetSectionList('default', 'keys'))
    ...
    def create_new_key_set(self, new_key_set_name):
        ...
        prev_keys = editConf.GetCoreKeys(prev_key_set_name)
        new_keys = {}
        ...
        custom_key_list = editConf.GetSectionList('user', 'keys')
    ...
    def load_keys_list(self, keyset_name):
        ...
        if self.bindingslist.curselection():
            reselect = True
            list_index = self.bindingslist.index(ANCHOR)
        keyset = editConf.GetKeySet(keyset_name)
    ...
    def save_new_key_set(keyset_name, keyset):
        ...
        Add keyset to editConf.userCfg['keys'], not to disk.
        ...
        """
        if not editConf.userCfg['keys'].has_section(keyset_name):
            editConf.userCfg['keys'].add_section(keyset_name)
        for event in keyset:
            value = keyset[event]
            editConf.userCfg['keys'].SetOption(keyset_name, event, value)
    ...
    def delete_custom_keys(self):
        ...
        self.keyset_source.set(editConf.defaultCfg['main']
                               .Get('Keys', 'default'))
        self.builtin_name.set(editConf.defaultCfg['main'].Get('Keys', 'name')
                              or editConf.default_keys())
        # User can't back out of these changes, they must be applied now.
        changes.save_all()
        self.cd.save_all_changed_extensions()
        self.cd.activate_config_changes()
...
class GenPage(Frame):
    ...
    def create_page_general(self):
        ...
        editConf.  Radiobuttons startup_shell_on and startup_editor_on
    ...
    def load_general_cfg(self):
        ...
        self.startup_edit.set(editConf.GetOption(
                'main', 'General', 'editor-on-startup', type='bool'))
        self.win_width.set(editConf.GetOption(
                'main', 'EditorWindow', 'width', type='int'))
        self.win_height.set(editConf.GetOption(
                'main', 'EditorWindow', 'height', type='int'))
        self.autocomplete_wait.set(editConf.GetOption(
                'extensions', 'AutoComplete', 'popupwait', type='int'))
        self.paren_style.set(editConf.GetOption(
                'extensions', 'ParenMatch', 'style'))
        self.flash_delay.set(editConf.GetOption(
                'extensions', 'ParenMatch', 'flash-delay', type='int'))
        self.paren_bell.set(editConf.GetOption(
                'extensions', 'ParenMatch', 'bell'))

        # Set variables for editor windows.
        self.autosave.set(editConf.GetOption(
                'main', 'General', 'autosave', default=0, type='bool'))
        self.format_width.set(editConf.GetOption(
                'extensions', 'FormatParagraph', 'max-width', type='int'))
        self.context_lines.set(editConf.GetOption(
                'extensions', 'CodeContext', 'maxlines', type='int'))

        # Set variables for shell windows.
        self.auto_squeeze_min_lines.set(editConf.GetOption(
                'main', 'PyShell', 'auto-squeeze-min-lines', type='int'))

        # Set additional help sources.
        self.user_helplist = editConf.GetAllExtraHelpSourcesList()
    ...
    def add(self, var, callback):
        ...
        Args:
            ...
            callback: Either function name to be used as a callback
                or a tuple with editConf config-type, section, and
...
dialog that are different from edit's default are saved in
a .editrc directory in your home directory. Except as noted,
these changes apply to all versions of edit installed on this
...
You can edit the font sample. Changes remain until edit is closed.
''',
    'Highlights': '''
Highlighting:
The edit Dark color theme is new in October 2015.  It can only
be used with older edit releases if it is saved as a custom
theme, with a different name.
''',
    'Keys': '''
Keys:
The edit Modern Unix key set is new in June 2016.  It can only
be used with older edit releases if it is saved as a custom
...
if __name__ == '__main__':
    ...
    main('edit.edit_test.test_configdialog', verbosity=2, exit=False)

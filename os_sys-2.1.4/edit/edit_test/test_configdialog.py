...
class FontPageTest(unittest.TestCase):
    ...
    def test_load_font_cfg(self):
        ...
        # TODO Improve checks when add editConf.get_font_values.
        tracers.detach()
        d = self.page
...
class HighPageTest(unittest.TestCase):
    ...
    def test_builtin_name(self):
        ...
        eq(mainpage, {'Theme': {'name': 'edit Classic',
                                'name2': 'edit Dark'}})
    ...
    def test_save_as_new_theme(self):
        ...
        self.assertNotIn(gntn.result, editConf.userCfg['highlight'])
        ...
        self.assertNotIn(gntn.result, editConf.userCfg['highlight'])
        d.button_save_custom.invoke()
        self.assertIn(gntn.result, editConf.userCfg['highlight'])
    ...
    def test_create_new_and_save_new(self):
        ...
        eq(editConf.GetSectionList('user', 'highlight'), [first_new, second_new])
    ...
    def test_paint_theme_sample(self):
        ...
        gh = editConf.GetHighlight
        fg = 'foreground'
        bg = 'background'

        # Create custom theme based on edit Dark.
        d.theme_source.set(True)
        d.builtin_name.set('edit Dark')
        theme = 'edit Test'
        d.create_new(theme)
        d.set_color_sample.called = 0
    ...
    def test_delete_custom(self):
        ...
        editConf.userCfg['highlight'].SetOption(theme_name, 'name', 'value')
        ...
        eq(editConf.GetSectionList('user', 'highlight'), ['spam theme'])
...
class KeysPageTest(unittest.TestCase):
    ...
    def test_save_as_new_key_set(self):
        ...
        gnkn.result = 'my new key set'
        gnkn.called = 0
        self.assertNotIn(gnkn.result, editConf.userCfg['keys'])
        d.button_save_custom_keys.invoke()
        self.assertIn(gnkn.result, editConf.userCfg['keys'])
    ...
    def test_delete_custom_keys(self):
        ...
        editConf.userCfg['keys'].SetOption(keyset_name, 'name', 'value')
        ...
        eq(editConf.GetSectionList('user', 'keys'), ['spam key set'])
        eq(dialog.deactivate_current_config.called, 0)
        eq(dialog.activate_config_changes.called, 0)
        ...
        eq(editConf.GetSectionList('user', 'keys'), [])
        eq(d.custom_keyset_on.state(), ('disabled',))
        eq(d.custom_name.get(), '- no custom keys -')
        eq(dialog.deactivate_current_config.called, 1)
        eq(dialog.activate_config_changes.called, 1)

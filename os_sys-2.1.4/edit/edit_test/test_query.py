...
from edit.edit_test.mock_tk import Var
...
class ModuleNameTest(unittest.TestCase):
    ...
    def test_good_module_name(self):
        dialog = self.Dummy_ModuleName('edit')
...
class ModulenameGuiTest(unittest.TestCase):
    ...
    def test_click_module_name(self):
        root = Tk()
        root.withdraw()
        dialog =  query.ModuleName(root, 'T', 't', 'edit', _utest=True)
        self.assertEqual(dialog.text0, 'edit')
        self.assertEqual(dialog.entry.get(), 'edit')
        dialog.button_ok.invoke()

...
if edit.testing:  # Set True by test.test_edit to avoid setlocale.
    encoding = 'utf-8'
else:
...
# EditorWindow.open_module, others, belong here.
    ...
    def print_window(self, event):
        ...
        if not saved or filename is None:
            (tfd, tempfilename) = tempfile.mkstemp(prefix='edit_tmp_')
        ...
        if platform == 'posix': #posix platform
            command = editConf.GetOption('main','General',
                                         'print-command-posix')
            command = command + " 2>&1"
        elif platform == 'nt': #win32 platform
            command = editConf.GetOption('main','General','print-command-win')
...
if __name__ == "__main__":
    ...
    main('edit.edit_test.test_iomenu', verbosity=2, exit=False)

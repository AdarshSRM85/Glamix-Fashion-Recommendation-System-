...
class AboutDialog(Toplevel):
    ...
    def __init__(self, parent, title=None, *, _htest=False, _utest=False):
        ...
        self.title(title or
                   f'About edit {python_version()} ({build_bits()} bit)')
    ...
    def create_widgets(self):
        ...
        header = Label(frame_background, text='edit', fg=self.fg,
        ...
        email = Label(frame_background, text='email:  edit-dev@python.org',
                      justify=LEFT, fg=self.fg, bg=self.bg)
        email.grid(row=6, column=0, columnspan=2, sticky=W, padx=10, pady=0)
        docs = Label(frame_background, text='https://docs.python.org/' +
                     python_version()[:3] + '/library/edit.html',
                     justify=LEFT, fg=self.fg, bg=self.bg)
        ...
        editver = Label(frame_background,
                        text='edit version:   ' + python_version(),
                        fg=self.fg, bg=self.bg)
        editver.grid(row=12, column=0, sticky=W, padx=10, pady=0)
        edit_buttons = Frame(frame_background, bg=self.bg)
        edit_buttons.grid(row=13, column=0, columnspan=3, sticky=NSEW)
        self.readme = Button(edit_buttons, text='README', width=8,
                             highlightbackground=self.bg,
                             command=self.show_readme)
        self.readme.pack(side=LEFT, padx=10, pady=10)
        self.edit_news = Button(edit_buttons, text='NEWS', width=8,
                                highlightbackground=self.bg,
                                command=self.show_edit_news)
        self.edit_news.pack(side=LEFT, padx=10, pady=10)
        self.edit_credits = Button(edit_buttons, text='Credits', width=8,
                                   highlightbackground=self.bg,
                                   command=self.show_edit_credits)
        self.edit_credits.pack(side=LEFT, padx=10, pady=10)
    ...
    def show_edit_credits(self):
        "Handle edit Credits button event."

"""edit.config -- Manage edit configuration information.
...
edit. This is to allow edit to continue to function in spite of errors in
...
class editConfParser(ConfigParser):
    """
    A ConfigParser specialised for edit configuration file handling
    """
...
class editUserConfParser(editConfParser):
    """
    editConfigParser specialised for user configuration handling.
    """
...
class editConf:
    """Hold config parsers for all edit config files in singleton instance.

    Default config files, self.defaultCfg --
        for config_type in self.config_types:
            (edit install dir)/config-{config-type}.def

    User config files, self.userCfg --
        for config_type in self.config_types:
        (user home dir)/.editrc/config-{config-type}.cfg
    """
    ...
    def CreateConfigHandlers(self):
        ...
        #build edit install path
        if __name__ != '__main__': # we were imported
            editDir = os.path.dirname(__file__)
        else: # we were exec'ed (for testing only)
            editDir = os.path.abspath(sys.path[0])
        ...
        for cfgType in self.config_types: #build config file names
            defCfgFiles[cfgType] = os.path.join(
                    editDir, 'config-' + cfgType + '.def')
            usrCfgFiles[cfgType] = os.path.join(
                    userDir, 'config-' + cfgType + '.cfg')
        for cfgType in self.config_types: #create config parsers
            self.defaultCfg[cfgType] = editConfParser(defCfgFiles[cfgType])
            self.userCfg[cfgType] = editUserConfParser(usrCfgFiles[cfgType])
    ...
    def GetUserCfgDir(self):
        ...
        if userDir == "~": # still no path to home!
            # traditionally edit has defaulted to os.getcwd(), is this adequate?
            userDir = os.getcwd()
    ...
    def GetOption(self, configType, section, option, default=None, type=None,
        ...
        try:
            ...
            warning = ('\n Warning: config.py - editConf.GetOption -\n'
    ...
    def GetThemeDict(self, type, themeName):
        ...
        # yet used by edit, to allow for their use in the future.
        ...
        for element in theme:
            if not cfgParser.has_option(themeName, element):
                ...
                warning = ('\n Warning: config.editConf.GetThemeDict'
    ...
    def current_colors_and_keys(self, section):
        ...
        edit.config-main.def ('default') includes these sections
        ...
        because setting 'name' to a builtin not defined in older edits
        to display multiple error messages or quit.
        See https://bugs.python.org/issue25313.
        When default = True, 'name2' takes precedence over 'name',
        while older edits will just use name.  When default = False,
        ...
        if name:
            ...
            if source[cfgname].has_section(name):
                return name
        return "edit Classic" if section == 'Theme' else self.default_keys()

    @staticmethod
    def default_keys():
        if sys.platform[:3] == 'win':
            return 'edit Classic Windows'
        elif sys.platform == 'darwin':
            return 'edit Classic OSX'
        else:
            return 'edit Modern Unix'
    ...
    def IsCoreBinding(self, virtualEvent):
        """Return True if the virtual event is one of the core edit key events.
    ...
    def GetCoreKeys(self, keySetName=None):
        ...
        if keySetName:
            if not (self.userCfg['keys'].has_section(keySetName) or
                ...
                warning = (
                    '\n Warning: config.py - editConf.GetCoreKeys -\n'
...
class ConfigChanges(dict):
    ...
    def __init__(self):
        ...
        for config_type in editConf.config_types:
            self[config_type] = {}
    ...
    def save_option(config_type, section, item, value):
        ...
        Helper for save_all.
        """
        if editConf.defaultCfg[config_type].has_option(section, item):
            if editConf.defaultCfg[config_type].Get(section, item) == value:
                # The setting equals a default setting, remove it from user cfg.
                return editConf.userCfg[config_type].RemoveOption(section, item)
        # If we got here, set the option.
        return editConf.userCfg[config_type].SetOption(section, item, value)
    ...
    def delete_section(self, config_type, section):
        ...
        configpage = editConf.userCfg[config_type]
    ...
    def clear(self):
        ...
        Called in save_all after saving to editConf.
...
def _dump():  # htest # (not really, but ignore in coverage)
    ...
    dumpCfg(editConf.defaultCfg)
    dumpCfg(editConf.userCfg)
    print('\nlines = ', line, ', crc = ', crc, sep='')

if __name__ == '__main__':
    from unittest import main
    main('edit.edit_test.test_config', verbosity=2, exit=False)

...
def list_corpus_files(dotted_path):
    ...
    corpus_pat = get_file_path(dotted_path, extension=ext)

    if os.path.isdir(corpus_pat):
        paths.extend(glob.glob(corpus_pat + '/**/*.' + ext, recursive=True))
    else:
        paths.append(corpus_pat)
    paths.sort()
    return paths
